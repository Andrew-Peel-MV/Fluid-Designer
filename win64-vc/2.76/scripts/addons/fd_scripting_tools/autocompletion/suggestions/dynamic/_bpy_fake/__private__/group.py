...
from . groupobjects import GroupObjects
from . bpy_struct import bpy_struct
import mathutils

class Group(bpy_struct):
    @property
    def rna_type(self):
    ...
    def preview(self):
        ...
        return ImagePreview()
    @property
    def dupli_offset(self):
        '''(Vector 3D) Offset from the origin to use when instancing as
        DupliGroup'''
        return mathutils.Vector()
    @property
    def layers(self):
        '''(Boolean[20]) Layers visible when this group is instanced as a dupli'''
        return bool()
    @property
    def objects(self):
        '''(Sequence of Object) A collection of this groups objects'''
        return GroupObjects()
    def copy(self):
