...
class Key(bpy_struct):
    ...
    @property
    def key_blocks(self):
        '''(Sequence of ShapeKey) Shape keys'''
        return (ShapeKey(),)
    @property
    ...
    @property
    def user(self):
        '''(ID) Datablock using these shape keys'''
        return ID()
    @property
    def use_relative(self):
        '''(Boolean) Make shape keys relative, otherwise play through shapes as a
        sequence using the evaluation time'''
        return bool()
    @property
    def eval_time(self):
        '''(Float) Evaluation time for absolute shape keys'''
        return float()
    def copy(self):
