...
BACKSPLASH_PART = (HIDDEN_FOLDER_NAME,"Cabinet Assemblies","Countertop Parts","Backsplash")
STRAIGHT_COUNTER_TOP = (HIDDEN_FOLDER_NAME,"Cabinet Assemblies","Countertop Parts","Countertop")
NOTCHED_COUNTER_TOP = (HIDDEN_FOLDER_NAME,"Cabinet Assemblies","Countertop Parts","Corner Countertop")

COUNTER_TOP_MATERIAL = ("Stone","Stone","Basalt Slate")

class Material_Pointers():
    
    Countertop_Surface = fd.Material_Pointer(COUNTER_TOP_MATERIAL)

class Cutparts_Pointers():
    
    pass

class Edgeparts_Pointers():
    
    pass

#---------PRODUCT: COUNTERTOPS
        
class PRODUCT_Straight_Countertop(fd.Library_Assembly):
    ...
    def __init__(self):
        ...
        self.height = fd.inches(5.5)
        self.depth = fd.inches(24)
        self.height_above_floor = fd.inches(34.1)
    ...
    def draw(self):
        ...
        self.add_prompt(name="Side Splash Setback",prompt_type='DISTANCE',value=fd.inches(2.75),tab_index=0)
        self.add_prompt(name="Edge Type",
                              ...
                              items=['Full Bull-Nose',
                                     ...
                                     'Dupont'],
                              columns=3)
        self.add_prompt(name="Deck Thickness",prompt_type='DISTANCE',value=fd.inches(1.5),tab_index=1)
        self.add_prompt(name="Splash Thickness",prompt_type='DISTANCE',value=fd.inches(.75),tab_index=1)
...
class PRODUCT_Notched_Corner_Countertop(fd.Library_Assembly):
    ...
    def draw(self):
        ...
        self.add_prompt(name="Left Side Depth",prompt_type='DISTANCE',value=fd.inches(24.0),tab_index=0)
        self.add_prompt(name="Right Side Depth",prompt_type='DISTANCE',value=fd.inches(24.0),tab_index=0)
        ...
        deck = self.add_assembly(NOTCHED_COUNTER_TOP) 
        deck.set_name("Countertop Deck")
        deck.x_loc(value = 0)
        deck.y_loc(value = 0)
        deck.z_loc(value = 0)
        ...
        deck.y_rot(value = 0)
        deck.z_rot(value = -90)
        deck.x_dim("fabs(Product_Depth)",[Product_Depth])
        deck.y_dim("Product_Width",[Product_Width])
        deck.z_dim("Deck_Thickness",[Deck_Thickness])
        deck.prompt("Left Side Depth","Left_Side_Depth",[Left_Side_Depth])
        deck.prompt("Right Side Depth","Right_Side_Depth",[Right_Side_Depth])
        deck.prompt("Edge Type","Edge_Type",[Edge_Type])
        deck.material("Countertop_Surface")
